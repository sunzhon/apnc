...
function sliderChange(ui,id,newVal)
    ...
    function graph_cb(msg)
        ...
        simSetGraphUserData(graphHandleLF,"GRF",data[92])
        --[[
        ...
        simSetGraphUserData(graphHandleLH,"GRF",data[93])
        --[[
    ...
    function sysCall_init()
        ...
        if (not pluginNotFound) then
            ...
            -- Start rosnode
            result=sim.launchExecutable(simGetStringParameter(sim_stringparam_scene_path) .. '/../../projects/genesis/catkin_ws/src/stbot_apnc/launch/start_lilibot_vrep',0)
        ...
        joint_pos, joint_vel, joint_force= {}, {},{}
        sensor_array = {}
        for i = 1, #joint_array do
            joint_pos[i] = simGetJointPosition(joint_array[i])
            sensor_array[i] = joint_pos[i]
        end
    ...
    function sysCall_sensing()
        ...
        for i = 1, #joint_array do  --
            joint_pos[i] = sensor_array[i] -- Save the joint postion of last step
        end

        for i = 1, #joint_array do -- joint position
            sensor_array[i]= simGetJointPosition(joint_array[i])
        end	

        for i = 1, #joint_array do -- joint verlocity
            -- Normally the freq = 60, if not please change it
            sensor_array[i+ #joint_array] = (sensor_array[i]-joint_pos[i])/60.0
        end

        for i = 1, #joint_array do -- joint force
            sensor_array[i + 2*#joint_array] = simGetJointForce(joint_array[i])
        end	

        for i = 1, #joint_array do -- joint voltage
        ...
        for i = 1, #foot_sensor_array do -- GRFs
            ...
            if(force==nil) 
                ...
                -- /16.0 normalization
                sensor_array[i + 4*#joint_array + #body_eularAngles + #body_position + #body_velocity] = math.sqrt(force[1]^2+force[2]^2+force[3]^2)/16.0 --+ ((force[3]/10.0 > 0.1) and 0.5*math.random() or 0.0) --let the value into [0,1] --force[3]/10.0 
                -- print(sensor_array[i+4*#joint_array+#body_eularAngles + #body_position + #body_velocity])
