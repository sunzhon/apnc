...
ModularNeural::ModularNeural(unsigned int nCPGs) {
    ...
    esnFMs.resize(nCPGs);
    phaseModulationGain=0.03;
    phaseResetThreshold=0.2;
    // 2) single module for whole robot
    // Distributed force feedback based reflex 
        dffbReflex = new DistributedForceReflex(0);
    ...
    for (unsigned int i = 0; i < nCPGs; i++) {
        ...
        //2.7-2) mIkineNNs , inverse kinematics
        ...
        //ii) Distributed force feedback based reflex with esn and learing 
...
void ModularNeural::step(unsigned int ID,CPGSTYPE CPGSType) {
    ...
    //1.5) update COG reflex or dffReflex
    ...
    mCPGs.at(ID)->setInput(grf.at(ID), np_grf.at(ID), ori, 0.0*phaseModulationGain + mAFGs.at(ID)->getOutput(),  phaseResetThreshold,  getmCPGsOutputs(n_CPGs),  CPGSType, cmatrix); //force feedback to this
    ...
    mAFGs.at(ID)->setInput(np_grf.at(ID),jmc.at(ID).at(1));
...
float ModularNeural::getCpgAsf(unsigned int ID, unsigned int idx)const{
    assert(ID <n_CPGs);
    assert(idx <2);
    return mCPGs.at(ID)->getASFOutput(idx);
}
float ModularNeural::getCpgAci(unsigned int ID, unsigned int idx)const{
    assert(ID <n_CPGs);
    assert(idx <2);
    return mCPGs.at(ID)->getACIOutput(idx);
}


void ModularNeural::setCPGModulationParameters(float gain, float threshold){
    phaseModulationGain=gain;
    phaseResetThreshold=threshold;
}
//-------------------PSN
