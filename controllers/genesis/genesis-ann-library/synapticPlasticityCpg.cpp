...
void SPCPG::updateActivities(){
    ...
    asf->setInput(asf_gain,asf_np_grf,o_t);
    asf->step();
    ...
    for(unsigned int i=0;i<2;i++){
        a_t1.at(i) = a_t.at(i) + aci->getOutput(i) - asf->getOutput(i) + prs->getOutput(i)  + 0.0*ves->getOutput(i)  + 0.0*pib->getOutput(i);
...
void ASF::setInput(parameter gamma,parameter grf,vectorD o_t){
    this->gamma=gamma;
    this->grf =grf;
    this->o_t =o_t;
}
...
ACI::ACI(unsigned int ID,unsigned int nCPGs){
    ...
    initK(0.00);
    initDelta();
    ACI_CPGSType=TROT;//default CPGStype is a trot
}
...
void ACI::setDelta(CPGSTYPE CPGSType, Matrix cmatrix){
    if(CPGSType!=ACI_CPGSType){// does the CPGSType changed
        ACI_CPGSType=CPGSType;
        switch(CPGSType){
            case GALLOP:
                // gallop output
                delta.at(0).at(0) = 0.0;
            ...
            case TROT: 
                ...
                delta.at(3).at(3) = 0.0;
                initK(0.01);
                cout<<"TROT was activated"<<endl;
                break;
            case DECOUPLED: // 5
                initK(0.0);
                cout<<"Decoupled was activated"<<endl;
                break;
            case SP:// synaptic plasticity 6, the delta was generated through tranfering the adaptive limb movement coordination relationship
