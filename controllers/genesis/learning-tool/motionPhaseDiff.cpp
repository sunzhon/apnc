...
//------------------------MotionPhaseDiffV3-------------------------//
//This class is using a limit cycle to calculate the phase difference//
//

MotionPhaseDiffV3::MotionPhaseDiffV3(unsigned int nCPGs){//the size of legs
    //1) initialize the connection matrix
    cmatrix.set(nCPGs,nCPGs);
    previous_cmatrix.set(nCPGs,nCPGs);
    // The number of CPGs
    this->nCPGs=nCPGs;
    //2) initialize the signals form pcpg
    signals.resize(nCPGs);
    for(unsigned int i=0;i<nCPGs;i++){
        signals.at(i).first= 0.0;
        signals.at(i).second= 0.0;
    }
}

MotionPhaseDiffV3::~MotionPhaseDiffV3(){
    ;
}

void MotionPhaseDiffV3::setInput(unsigned int index, SO2CPGOuts sig){
    assert(index <nCPGs);
    signals.at(index)=sig;
}

void MotionPhaseDiffV3::step(){
    ...
    for(unsigned int i=0; i< nCPGs; i++){
        for(unsigned int j=0;j<nCPGs;j++){
            if(j>i){// 只计算上三角矩阵
                temp1= signals.at(i).first*signals.at(j).first+signals.at(i).second*signals.at(j).second;
                temp2 = sqrt(pow(signals.at(i).first,2)+pow(signals.at(i).second, 2))*sqrt(pow(signals.at(j).first, 2)+pow(signals.at(j).second, 2));
                if (temp2 > 0.0){
                    temp3 = temp1/temp2;
                }else{// assume them in phase
                    perror("Phase relationship evaluation error, the input signals are not right!");
                    temp3 = 1.0;
                }
                ...
                if(temp3 <-1.0)
                    temp3=-1.0;
                cmatrix.val(i,j) = acos(temp3);
            }
            else if(i==j){// i=j, itself phase relationship
                cmatrix.val(i,j) = 0.0;
            }
            else{// 下三角和上三角相反数
                cmatrix.val(i,j) = -cmatrix.val(j,i);
            }
        }
    }
    //2) low-pass filter
    for(unsigned int i=0; i< nCPGs; i++){
        for(unsigned int j=0;j<nCPGs;j++){
            cmatrix.val(i,j)=low_pass_rate*cmatrix.val(i,j)+(1-low_pass_rate)*previous_cmatrix.val(i,j);
        }
    }
    previous_cmatrix=cmatrix;
}

Matrix MotionPhaseDiffV3::getOutput(){
    return cmatrix;
}
