...
typedef	std::pair<float, float> SO2CPGOuts;
//-------------------QueueVessel------------------------//
template<typename T>
class QueueVessel{
public:
...
//-----------------MotionPhaseDiffV3-----------------//
class MotionPhaseDiffV3{
public:
	MotionPhaseDiffV3(unsigned int nCPGs);
	~MotionPhaseDiffV3();
...
private:
    unsigned int nCPGs;
	Matrix cmatrix;
	Matrix previous_cmatrix;
    const float low_pass_rate=0.3;
	std::vector<SO2CPGOuts> signals;
