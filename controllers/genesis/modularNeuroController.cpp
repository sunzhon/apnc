...
//plotData plot("2",4);
bool ANC=true; // adaptive neural communication
...
namespace stcontroller {
    ...
    void ModularNeuroController::step(const parameter* x_, int number_sensors, parameter* y_, int number_motors) {
        ...
        // get the ASF and ACI outputs, they modules values
 ...
 */
        ...
        modules_data.push_back(conf.stFMOut.at(0));// forward model
        modules_data.push_back(conf.stFMOut.at(1));//forward model
        modules_data.push_back(mnc->getAFGOutput(0));//adaptive feedback gain
        modules_data.push_back(mnc->getAFGOutput(1));//adaptive feedback gain
